@echo off
rem     build.bat: Build XPath API packages
...
rem Except, of course, the JavaCC.jar file which must be present too
set _ANT_JAR=%ANT_JAR%
if "%_ANT_JAR%" == "" set _ANT_JAR=%_ANT_HOME%\bin\ant.jar;%_ANT_HOME%\bin\optional.jar
...
if "%_XALAN_JAR%" == "" set _XALAN_JAR=%_ANT_HOME%\bin\xalan.jar
...
@echo on
"%_JAVACMD%" -mx64m %JAVA_OPTS% -Dant.home="%_ANT_HOME%" -classpath "%_CLASSPATH%" org.apache.tools.ant.Main -f build.xml %1 %2 %3 %4 %5 %6 %7 %8 %9
@echo off
